...
이게 없으면 Next 어댑터가 붙지 않은 것이고, 그러면 `.next` 폴더가 정적 파일로
서빙되어 **모든 페이지가 404** 가 됩니다. 겉보기에 빌드는 성공으로 표시되기
때문에 이 섹션 유무로 판단해야 합니다.

증상으로 구분하는 방법 — `/BUILD_ID` 나 `/routes-manifest.json` 이 200 으로
열리면 어댑터가 안 붙은 상태입니다. 원래는 둘 다 외부에 노출되지 않습니다.
