...
// forked from design-sync lib/source-kit.mjs — Next.js 앱 형태: componentSrcMap null 컴포넌트의 파일을 synth 엔트리에서 제외 + 브라우저 process 심
...
export async function resolvePackage(ctx) {
  ...
  if (!entry) {
    ...
    const nullNames = new Set(Object.entries(srcMap).filter(([, v]) => v === null).map(([k]) => k));
    const pascalOf = (p) => p.split('/').pop().replace(/\.(tsx|jsx|mdx?)$/, '').split(/[-_]/).map((w) => w.charAt(0).toUpperCase() + w.slice(1)).join('');
    const comps = srcFiles.filter((p) => SRC_IMPL_RX.test(p) && !NON_IMPL_RX.test(p) && !nullNames.has(pascalOf(p)));
    entry = join(OUT, '.pkg-entry.mjs');
    // ESM 은 import 를 먼저 평가하므로 process 심은 별도 모듈로 만들어 첫 import 로 둔다
    const shim = join(OUT, '.process-shim.mjs');
    writeFileSync(shim, 'globalThis.process = globalThis.process || { env: {} };\n');
    // realpath 로 풀어야 esbuild 가 node_modules 밖 파일로 보고 tsconfig paths(next/navigation 심)를 적용한다
    writeFileSync(entry, `import ${JSON.stringify(resolve(shim))};\n` + comps.map((p) => `export * from ${JSON.stringify(realpathSync(p))};`).join('\n') + '\n');
    synthEntry = true;
    console.error(
